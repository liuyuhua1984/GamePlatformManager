...
import com.lyh.game.mybatis.domain.UserPay;
import com.lyh.game.mybatis.mapper.UserMapper;
import com.lyh.game.mybatis.mapper.UserPayMapper;
 ...
 */
@Service
public class UserPayService implements IBaseService<UserPay>, IPageHelper<UserPay> {
	...
	public long insert(UserPay t) {
		
		if (t == null) {
			logger.error("UserPay is null");
			return 0;
		}
	...
	public long insertBatch(List<UserPay> list) {
		
		if (list == null || list.size() <= 0) {
			logger.error("insertList<UserPay>为空或者size为0");
			return 0;
		}
		long result = 0;
		try {
			...
			// e.printStackTrace();
			logger.error("插入多个UserPay异常::", e);
		}
		return result;
	}
	
	public long update(UserPay t) {
		if (t == null) {
			logger.error("更新UserPay为空");
			return 0;
		}
		long result = 0;
		try {
			...
			// e.printStackTrace();
			logger.error("更新UserPay异常::", e);
		}
		
		return result;
	}
	
	public long updateBatch(List<UserPay> list) {
		
		if (list == null || list.size() <= 0) {
			logger.error("更新多个UserPay为空::");
			return 0;
		}
	...
	public UserPay getById(long id) {
		try {
			return mapper.getById(id);
		} catch (Exception e) {
			// e.printStackTrace();
			logger.error("查找UserPay异常::" + id, e);
		}
		return null;
	}
	...
	@Override
	public PageEntity<UserPay> listByPage(Map<String, Object> paramMap, int startPage, int numPerPage) throws Exception {
		try {
			if (paramMap == null || paramMap.isEmpty()){
				logger.error("UserPay分页参数为空::");
				return null;
			}
			PageHelper.startPage(startPage, numPerPage);
			List<UserPay> list = listBy(paramMap);
			long total = ((Page<UserPay>) list).getTotal();
			return new PageEntity<UserPay>(total, list, startPage, numPerPage);
		} catch (Exception e) {
	...
	public List<UserPay> listBy(Map<String, Object> paramMap) {
		try {
			if (paramMap == null || paramMap.isEmpty()) {
				logger.error("查询多个UserPay条件为空::");
				return null;
			}
			return mapper.listBy(paramMap);
		} catch (Exception e) {
			// e.printStackTrace();
			logger.error("条件查询多个UserPay异常::" + paramMap.toString(), e);
		}
		return null;
	}
	
	public UserPay getBy(Map<String, Object> paramMap) {
		try {
			if (paramMap == null || paramMap.isEmpty()) {
				logger.error("查询单个UserPay条件为空::");
				return null;
			}
			return mapper.getBy(paramMap);
		} catch (Exception e) {
			logger.error("条件查询单个UserPay异常::" + paramMap.toString(), e);
		}
		return null;
	}
