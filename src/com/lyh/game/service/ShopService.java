...
import com.lyh.game.mybatis.domain.Shop;
import com.lyh.game.mybatis.domain.User;
import com.lyh.game.mybatis.mapper.ShopMapper;
 ...
 */
@Service
public class ShopService implements IBaseService<Shop>, IPageHelper<Shop> {
	...
	public long update(Shop t) {
		if (t == null) {
			logger.error("更新Shop为空");
			return 0;
		}
		long result = 0;
		try {
	...
	@Override
	public PageEntity<Shop> listByPage(Map<String, Object> paramMap, int startPage, int numPerPage) throws Exception {
		try {
			if (paramMap == null || paramMap.isEmpty()){
				logger.error("Shop分页参数为空::");
				return null;
			}
			PageHelper.startPage(startPage, numPerPage);
			List<Shop> list = listBy(paramMap);
			long total = ((Page<Shop>) list).getTotal();
			return new PageEntity<Shop>(total, list, startPage, numPerPage);
		} catch (Exception e) {
			logger.error("Shop分页出现异常::", e);
		}
		return null;
	}
	...
	public List<Shop> listBy(Map<String, Object> paramMap) {
		try {
			if (paramMap == null || paramMap.isEmpty()) {
				logger.error("查询多个Shop条件为空::");
				return null;
			}
			return mapper.listBy(paramMap);
		} catch (Exception e) {
			// e.printStackTrace();
			logger.error("条件查询多个Shop异常::" + paramMap.toString(), e);
		}
		return null;
	}
	...
	public Shop getBy(Map<String, Object> paramMap) {
		try {
			if (paramMap == null || paramMap.isEmpty()) {
				logger.error("查询单个Shop条件为空::");
				return null;
			}
			return mapper.getBy(paramMap);
		} catch (Exception e) {
			logger.error("条件查询单个Shop异常::" + paramMap.toString(), e);
		}
		return null;
	}
